...
class AiPageBuilderService
{
    public static function buildMessages(array $page, string $message, array $selectedElement, string $domSnapshot): array
    {
        ...
        $systemPrompt = implode("\n", [
            ...
            '- You may ONLY edit the page schema/blocks for the current page.',
            '- You MUST NOT output PHP, server code, shell commands, SQL, or anything targeting the codebase or filesystem.',
            '- You MUST NOT output <script>, <iframe>, <object>, <embed>, inline event handlers (onclick, onload, onerror, ...), or external resource loaders inside any block content.',
            '- You MUST NOT reference admin routes, settings, users, roles, permissions, or the database.',
            '- You MUST NOT include javascript: URLs or executable data: payloads.',
            '- If asked to do anything outside editing the page schema, refuse politely in the summary field.',
            '',
            'CORE PRINCIPLES:',
            '1. AI-FIRST CREATION',
