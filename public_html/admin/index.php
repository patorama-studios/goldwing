  ...
  if (!Csrf::verify($_POST['csrf_token'] ?? '')) {
    ...
    if ($page === 'applications' && isset($_POST['approve_id'])) {
      ...
      if (!$row) {
        ...
        $associateAdd = trim((string) ($notes['membership']['associate_add'] ?? 'no'));
        $fullTerm = $term;
        $associateTerm = $term;
        if ($fullSelected) {
    ...
    if ($page === 'fallen-wings' && ($_POST['action'] ?? '') === 'edit_fallen_wings') {
      ...
      if ($entryId > 0 && $fullName !== '' && $year > 0) {
        $uploadDir = __DIR__ . '/../uploads/fallen_wings/';
        if (!is_dir($uploadDir)) {
          mkdir($uploadDir, 0755, true);
        }

        $sql = 'UPDATE fallen_wings SET full_name = :full_name, year_of_passing = :year_of_passing, member_number = :member_number, tribute = :tribute, updated_at = NOW()';
        $params = [
          'full_name' => $fullName,
        ...
        if (isset($_FILES['tribute_image']) && $_FILES['tribute_image']['error'] === UPLOAD_ERR_OK) {
          ...
          if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'])) {
            ...
            if (move_uploaded_file($_FILES['tribute_image']['tmp_name'], $uploadDir . $filename)) {
              ...
              $params['image_url'] = '/uploads/fallen_wings/' . $filename;
            }
          }
        }

        if (isset($_FILES['tribute_pdf']) && $_FILES['tribute_pdf']['error'] === UPLOAD_ERR_OK) {
          ...
          if ($ext === 'pdf') {
            ...
            if (move_uploaded_file($_FILES['tribute_pdf']['tmp_name'], $uploadDir . $filename)) {
              $sql .= ', pdf_url = :pdf_url';
              $params['pdf_url'] = '/uploads/fallen_wings/' . $filename;
            }
          }
        }
        ...
        $stmt->execute($params);
        AuditService::log($user['id'], 'fallen_wings_update', 'Memorial #' . $entryId . ' updated.');
        $alerts[] = ['type' => 'success', 'message' => 'Memorial entry updated successfully.'];
      } else {
    ...
    if ($page === 'wings' && ($_POST['action'] ?? '') === 'edit_wings_issue') {
      ...
      if ($title && $publishedAt && $issueId > 0) {
        if ($isLatest) {
          $pdo->query('UPDATE wings_issues SET is_latest = 0');
        }
    ...
    if ($page === 'wings' && ($_POST['action'] ?? '') === 'delete_wings_issue') {
      $issueId = (int) ($_POST['issue_id'] ?? 0);
      if ($issueId > 0) {
        $stmt = $pdo->prepare('DELETE FROM wings_issues WHERE id = :id');
        $stmt->execute(['id' => $issueId]);
        $alerts[] = ['type' => 'success', 'message' => 'Wings issue deleted.'];
      }
    }
  }
}
  ...
  <main class="flex-1 overflow-y-auto bg-background-light relative">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <?php foreach ($alerts as $alert): ?>
        <div
          class="rounded-lg px-4 py-2 text-sm <?= $alert['type'] === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700' ?>">
          <?= e($alert['message']) ?>
        </div>
      <?php endforeach; ?>
      ...
      <?php elseif ($page === 'members'): ?>
        <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm text-sm">
          ...
          <p class="text-gray-600 mb-4">
            The legacy members section has moved to <a class="font-semibold text-primary"
              href="/admin/members">/admin/members</a>.
            Please use the new Members area for directory management, orders, refunds, and reporting.
          </p>
          <div class="flex flex-wrap gap-2">
            <a class="inline-flex items-center px-4 py-2 rounded-lg border border-gray-200 text-sm font-semibold text-gray-700"
              href="/admin/members">Open Members area</a>
          </div>
        </section>
            ...
            <div class="flex flex-wrap items-center gap-2">
              <?php foreach ($statusLabels as $statusKey => $statusMeta): ?>
                <a class="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-semibold <?= $statusFilter === $statusKey ? 'border-gray-900 text-gray-900' : 'border-gray-200 text-gray-600 hover:border-gray-300' ?>"
                  ...
                  <?= e($statusMeta['label']) ?>
                  <span
                    class="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600"><?= e((string) $statusCounts[$statusKey]) ?></span>
                </a>
              <?php endforeach; ?>
            </div>
          </div>
              ...
              <tbody class="divide-y">
                <?php foreach ($applicationRows as $row): ?>
                  ...
                  <tr>
                    ...
                    <td class="py-4 pr-4">
                      <span
                        class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold <?= $statusMeta['classes'] ?>">
                        <?= e($statusMeta['label']) ?>
                      </span>
                    </td>
                    ...
                    <td class="py-4 pr-4">
                      <form method="post" class="flex flex-col gap-2">
                        ...
                        <button
                          class="inline-flex items-center justify-center rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-700 hover:border-gray-300"
                    ...
                    <td class="py-4 pr-4">
                      ...
                      <?php else: ?>
                        <span
                          class="inline-flex items-center justify-center rounded-full border border-emerald-200 bg-emerald-50 text-emerald-700 h-9 w-9 opacity-50 cursor-not-allowed"
                    ...
                    <td class="py-4 pr-4">
                      <?php if ($canReject): ?>
                        <button type="button"
                          class="inline-flex items-center justify-center rounded-full border border-rose-200 bg-rose-50 text-rose-700 h-9 w-9 hover:bg-rose-100"
                          data-reject-open data-app-id="<?= e((string) $application['id']) ?>"
                          data-app-name="<?= e($row['display_name']) ?>">
                          <span class="material-icons-outlined text-base">close</span>
                        </button>
                      <?php else: ?>
                        <span
                          class="inline-flex items-center justify-center rounded-full border border-rose-200 bg-rose-50 text-rose-700 h-9 w-9 opacity-50 cursor-not-allowed"
                          title="Cannot reject">
                          <span class="material-icons-outlined text-base">close</span>
                        </span>
                      <?php endif; ?>
                    ...
                    <td class="py-4 pr-4">
                      <form method="post">
                        ...
                        <button
                          class="inline-flex items-center justify-center rounded-full border border-blue-200 bg-blue-50 text-blue-700 h-9 w-9 hover:bg-blue-100 <?= $application['status'] !== 'APPROVED' ? 'opacity-50 cursor-not-allowed' : '' ?>"
                          type="submit" <?= $application['status'] !== 'APPROVED' ? 'disabled' : '' ?>
        ...
        <dialog id="approve-dialog" class="rounded-2xl border border-gray-200 shadow-xl p-0 w-full max-w-lg">
          <form method="post" class="p-6 space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <button
                class="inline-flex items-center rounded-lg border border-gray-200 px-4 py-2 text-xs font-semibold text-gray-700"
                type="button" data-dialog-close>Cancel</button>
            </div>
          </form>
        </dialog>
        ...
        <dialog id="reject-dialog" class="rounded-2xl border border-gray-200 shadow-xl p-0 w-full max-w-lg">
          <form method="post" class="p-6 space-y-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600">Provide a one-line reason for rejecting <span class="font-semibold"
                  id="reject-name"></span>.</p>
            </div>
            ...
            <div class="flex flex-wrap gap-2">
              <button class="inline-flex items-center rounded-lg bg-rose-600 px-4 py-2 text-xs font-semibold text-white"
        ...
        <section class="space-y-6">
          <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="inline-flex items-center rounded-lg border border-gray-200 bg-white p-1 text-sm">
              <button type="button" data-notice-tab="live" class="px-3 py-1.5 rounded-md font-semibold text-gray-700">Live
                notices</button>
              <button type="button" data-notice-tab="pending"
                class="px-3 py-1.5 rounded-md font-semibold text-gray-700">Pending requests</button>
            </div>
          </div>
                  ...
                  <div class="border border-gray-100 rounded-xl p-4 space-y-3">
                    ...
                    <form method="post" class="space-y-2">
                      ...
                      <button
                        ...
                        type="submit">Reject with reason</button>
                    </form>
                  </div>
                <?php endforeach; ?>
              <?php else: ?>
