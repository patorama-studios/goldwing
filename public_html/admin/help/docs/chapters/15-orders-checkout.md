...
**Orders with no membership period (Aug 2026).** `membership_period_id` can be NULL on a membership order — the admin **Request payment** top-up creates one that way, and a signup order carries none until approval. `activateMembershipForOrder()` used to return `false` for those, so a card payment landed with the order marked *paid* and the member left with no membership period and **no renewal date**. It now mints the missing period first (`createPeriodForOrphanOrder()`), reading the term off the order's line items, then calculates the end date as normal. It refuses to mint for a `PENDING` applicant, a life member, or an order whose items don't mention membership/renewal/join — the Request-payment description is free text, and a one-off charge must not extend cover. See [19 — Membership lifecycle](view.php?slug=19-membership-lifecycle) for the repair tool covering members already stuck undated.

Admin visibility mirrors the bank-transfer flow: the moment a member reaches the Stripe payment form, `MembershipInvoiceService` fires the `membership_admin_stripe_pending` email to the admin/treasurer recipients (first attempt per order only — refreshes stay quiet), the successful charge fires `membership_treasurer_notification`, and a failed / cancelled payment fires `membership_admin_payment_issue` (see Ch 22).
...
- **The order row is created BEFORE Stripe confirms.** Both flows insert `pending`, then wait for the webhook to flip to `paid`. Abandoned checkouts leave harmless `pending` rows — filter `status != 'pending'` for "real orders".
- **Store checkout reuses one pending order per cart (idempotent).** The on-page Payment Element calls `/api/stripe/create-payment-intent` whenever the card step opens — including on every page reload or revisit. That used to INSERT a fresh `store_orders` row (+ mirror `orders` row + admin "order placed" email) each time, so one paid order ended up surrounded by N pending duplicates. Now each store order is stamped with its `store_orders.cart_id`; the endpoint reuses the cart's existing pending order (handing back the same Stripe invoice) when the total is unchanged, and only cancels-and-rebuilds when the cart total actually changes. Fixed via Migration 036. Legacy duplicates (cart_id `NULL`) are left for the cleanup cron / tool.
...
- **Stale pending cleanup is automated (now a safety net, not the fix).** `cron/expire_pending_orders.php` ([Chapter 34](view.php?slug=34-cron-jobs)) closes off any card-method `pending` order older than 24h, mirroring `MembershipOrderService::markOrderRejected` for the linked `membership_periods` row. Since Migration 036 made store checkout idempotent per cart, new duplicates no longer accumulate — this cron and `/admin/cleanup-stuck-store-orders.php` now mostly just mop up legacy rows and the rare genuinely-abandoned checkout. Don't clean those rows by hand — let the cron do it.
