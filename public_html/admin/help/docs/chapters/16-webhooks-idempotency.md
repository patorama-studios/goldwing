...
- **A member paid in Stripe but their membership didn't activate.** As a safety net (added after the July 2026 renewal incident), a membership renewal is **only** activated when the amount Stripe actually collected covers the full membership order(s). If an invoice comes back paid for *less* than it should be, activation is deliberately **blocked** rather than granting membership for an underpayment. Look in the activity log (Admin → Security Log) for `membership.activation_blocked_underpaid` — the entry records what was paid vs what was owed. This normally means the Stripe invoice was built wrong (a missing line item); check the member's invoice in Stripe, then have the renewal re-done through the member's portal, which now refuses to start a payment whose total doesn't match what the member is shown.
