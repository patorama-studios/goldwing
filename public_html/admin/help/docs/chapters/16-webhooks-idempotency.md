...
- **A member paid in Stripe but their membership didn't activate.** Two distinct causes:
  1. **The webhook never reached us (the July 2026 outage).** The `/api/*` admin access gate returned 401 to Stripe's anonymous `invoice.paid` POST, so activation never ran — the card was charged, but the order stayed PENDING, no receipt went out, and the confirmation popup showed the member's *old* coverage. The gate is now whitelisted so new renewals are fine. To clean up anyone stranded in the window, use **`/admin/reconcile-stranded-payments.php`**: it lists every non-paid membership order that has a Stripe invoice, checks the *real* invoice in Stripe, and — only for ones Stripe confirms as paid — replays the normal activation (correct term, order marked paid, receipt sent). Safe to re-run (idempotent), and it won't touch invoices where a sibling order already activated (partial heal — those it flags for manual review).
  2. **Deliberately blocked as underpaid.** As a safety net, a membership renewal is **only** activated when the amount Stripe actually collected covers the full membership order(s). If an invoice comes back paid for *less* than it should be, activation is deliberately **blocked** rather than granting membership for an underpayment. Look in the activity log (Admin → Security Log) for `membership.activation_blocked_underpaid` — the entry records what was paid vs what was owed. This normally means the Stripe invoice was built wrong (a missing line item); check the member's invoice in Stripe, then have the renewal re-done through the member's portal, which now refuses to start a payment whose total doesn't match what the member is shown.
...
| `payment_intent.succeeded` | `handlePaymentIntentSucceeded()` | **Primary event for the inline membership Payment Element flow** (pay-membership lightbox) + safety net for store. Marks the `metadata.order_id` order paid, and for membership orders calls `markMembershipPaid()` (period → ACTIVE, member receipt, treasurer notification; it needs only a `member_id` — activation mints a missing period, and a genuine failure is now recorded via `StripeErrorLogger::logWebhookSkip()` instead of passing silently) — there is no `checkout.session.completed` in the Element flow. Also processes `metadata.extra_order_ids` (comma-separated): one PI can pay several orders, e.g. the partner's renewal bought in the same lightbox — each extra order gets marked paid, activated, and invoiced. Skipped when `intent.invoice` is set — those events flow through `invoice.paid` below to keep the store invoice + PI in sync. Closes out the cart for store orders. The body lives in the public `reconcilePaymentIntent()` — `/member/` page loads also call it synchronously when a pending membership order's PI turns out to have already succeeded (webhook delayed or missed), so activation never depends solely on webhook delivery. |
| `invoice.paid` | `handleInvoicePaid()` | **Primary event for store orders.** Resolves the order via `stripe_invoice_id` → `stripe_subscription_id` → `metadata.order_id`; for `order_type=store` rows hands off to `markStoreOrderPaid()` (cart conversion + stock + tickets + invoice email). For `order_type=membership` rows activates the membership period. |
