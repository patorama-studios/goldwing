...
- **Unnumbered members hold `NULL`, not `0` (Migration 048).** `members` has `UNIQUE (member_number_base, member_number_suffix)`, so a `0-0` placeholder allowed only ONE pending unnumbered member at a time — the second application died with `Duplicate entry '0-0'` *after* the applicant's card was charged (the July 2026 stranded-application bug). Pending members now insert `member_number_base = NULL` (unique indexes ignore NULL rows). Never reintroduce a `0` placeholder in a member INSERT.
- **Life status is two-place (Migration 049).** `members.is_life_member` marks an **associate** who holds life membership — `member_type` stays `ASSOCIATE` so the household link and `.1` number keep working; full life members remain `member_type = 'LIFE'` with the flag at 0. Never test `member_type = 'LIFE'` alone for "never lapses" logic — use `MemberRepository::isLifeMember()` / `notLifeSql()` (the SQL helper omits the flag clause until the column exists, so it's safe to call before the migration has run). The migration also seeds the "Associate Life" `membership_types` row the admin dropdown offers.
- **Postage exemption is per product (Migration 050).** `store_products.free_shipping` marks an item the quartermaster posts free (stickers, badges). It is an *exemption*, not a per-product rate — the flat rate is still charged once per order, and only when the cart holds a `physical` product with the flag at 0. `store_cart_needs_postage()` reads the column on every cart-totals call, so deploying that code without running the migration will fatal at checkout.
- **Page views are opt-in by migration (Migration 051).** `page_views` is written by `PageViewLogger` on every HTML page load once the table exists; until then the logger swallows the missing-table error silently and Admin → Reports shows a "tracking is not switched on yet" notice. Nothing else depends on the table. See [Chapter 39 — Member engagement report](view.php?slug=39-engagement-reports).
- **`application_drafts` is created lazily**, not by a migration — `ApplicationDraftService` issues `CREATE TABLE IF NOT EXISTS` on first use so the pre-payment form snapshot can never be missing on a fresh deploy. Don't add it to `schema.sql` checklists as a manual step; it manages itself (30-day self-sweep).
