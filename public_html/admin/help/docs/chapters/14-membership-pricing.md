...
`config/app.php` still declares `stripe.membership_prices` (`FULL_1Y`, `FULL_3Y`, `ASSOCIATE_1Y`, `ASSOCIATE_3Y`, `LIFE`), read by `cron/send_renewal_reminders.php` to build "renew now" email links. A separate Checkout Session route at `public_html/api/index.php:~753` and the **member-initiated renewal lightbox** at `public_html/member/index.php` (handler: `$_POST['action'] === 'membership_renew'`) both read `payments.membership_prices` (`FULL_12`, `FULL_24`, `FULL_36`, `ASSOCIATE_12`, `ASSOCIATE_24`, `ASSOCIATE_36`) from `StripeSettingsService`. The member-renewal handler also accepts the legacy `_1Y` / `_3Y` keys as a fallback via the `membership_stripe_price_id()` helper. For new applications, `/apply` still uses PaymentIntent + the pricing matrix — the Stripe Price IDs only matter for renewals (cron + member modal) and the legacy Checkout Session API. See [13 — Stripe integration overview](view.php?slug=13-stripe-overview) and [19 — Membership lifecycle](view.php?slug=19-membership-lifecycle). The 36-month term only appears in the public dropdown when at least one of `FULL_36` / `ASSOCIATE_36` is set; otherwise it stays hidden.
