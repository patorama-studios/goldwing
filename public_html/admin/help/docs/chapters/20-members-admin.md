...
**To search**, type into the search box at the top — it matches name, email, and member number. To narrow further use the filter chips: chapter (including a **No chapter assigned** option to surface members who never got placed into one), status, membership type, role, directory preferences (the A–F flags), vehicle type, signup date range, **Expiring** (within 30 / 60 / 90 days, before the next 31 July renewal, past expiry but still in the grace period, or already expired), and **Renewed within** a date range (under Advanced filters — enter a "Renewed from" and/or "Renewed to" date to list members who made a membership payment in that window; either date works on its own). Every stat card above the list is a clickable filter shortcut: **Total members** clears the filters, **Active** / **Pending** / **Expired** apply that status, **Past expiry (grace)** shows members whose membership has ended but who are still inside the grace window (they haven't renewed and haven't been flipped to lapsed yet — the cohort to contact during renewals), **Expiring (60d)** and **New (30d)** apply those windows, and **Renewals** shows members who paid a membership order this calendar month. The card matching the current view is highlighted, and clicking one keeps any text search you've typed while resetting the other filters.
...
1. **Type & number** — Full / Associate / Life, chapter, and the number. For Full / Life the next free member number is suggested (override if you need to). For an **Associate** the link comes first: the member-number box disappears and the wizard asks **who the associate is linked to**. Pick the full member and an **Associate number** field appears showing their base with the point suffix (e.g. `1494 . 2`) — the next free point is suggested, editable if the household has a specific one in mind. Pick **"No link (own number)"** instead to give the associate their own stand-alone number (the normal number box returns). The wizard won't move past this step until one or the other is chosen. **A member can only carry one associate at a time**: full/life members who already have one are greyed out in the picker and labelled with the associate's name, and the server rejects the submit as a backstop. Archiving (cancelling) an associate frees the slot; otherwise unlink the current one first (member page → Overview → Associate Member → **Unlink**).
2. **Contact** — first name and last name are required; email and phone are optional (rare, but some members have neither — just leave them blank; without an email the welcome/payment emails on the Finish step can't send). If the email is already on another member (common for households sharing one address), the wizard says who owns it and offers **"Link as associate & share email"** — accepting switches the type to Associate, pre-selects that household's Full/Life member as the link, and lets the shared email through. Decline to type a different email instead. If that household already has an associate, the prompt says so and the link shortcut is withheld.
3. **Address** — all optional. For a linked associate this step arrives pre-filled with the linked member's address (households usually share one) — edit anything that differs; the prefill never overwrites something you've already typed, and un-linking clears only what it filled in.
...
7. **Finish** — a review summary first (name, type, member number, and — for an associate — who they're linked to, or "Not linked" if they get their own number), then the member and membership are always created; the checkboxes choose what to send:
...
On submit the wizard itself confirms the result — a full-page green tick reading "**\<name\> has been added to the system**" with the member number, the household link for an associate ("linked as an associate of …"), what was created, which emails went out, and **View member** / **Add another member** buttons. If the server rejects the submit (duplicate email, member number in use, associate slot taken), the error appears next to the buttons **with everything you typed still in place** — fix the field and submit again; you are no longer bounced back to step 1 with an empty form. (Without JavaScript the wizard falls back to a full-page submit that lands on the new member's Overview with the same confirmation as a banner.)
...
- **Make an associate a life member ("Associate Life")** — open the associate → Orders tab → **Membership & Admin** panel → set **Membership type** to **Associate Life** → Save. They stay a linked associate (same `.1` number, same household) but never lapse and get no renewal reminders; their renewal date shows N/A and the full member's renewal no longer offers them as a partner add-on. The dropdown only switches life status on and off — it refuses structural moves (a linked associate can't become Full/Life-in-their-own-right from here, and a numbered member can't become an associate; use the unlink/upgrade flows for those).
...
Reads filters from the query string (`q`, `status`, `chapter_id` (with the sentinel `0` meaning *no chapter assigned* → `m.chapter_id IS NULL`), `membership_type_id`, `role`, `directory_pref[]`, `vehicle_*`, `created_range`, `expiring_within` (`30d` / `60d` / `90d` / `eoy` = before next 31 July / `overdue` = latest ACTIVE `end_date` in the past but `members.status` still active, i.e. the grace-period cohort the expire cron hasn't flipped to LAPSED yet / `expired`), `renewed` (`this_month` = has a paid membership order this calendar month, an `EXISTS` subquery on `orders` mirroring the renewals stat), `renewed_from` / `renewed_to` (the **Renewed within** advanced filter — the same `EXISTS` on paid membership `orders` but bounded by `paid_at` between the given dates; either bound is optional), `sort_by`, `sort_dir`), hands them to `MemberRepository::search`, renders a table. There is no pagination — the list shows every matching member in one scroll (the whole club fits comfortably on one page; old `page=`/`limit=` bookmarks are simply ignored). The `expiring_within` filter compares the member's **latest** ACTIVE `end_date` (a `MAX(end_date)` subquery against `membership_periods`) to the chosen window — not just *any* active period, because after a renewal the old period stays ACTIVE until the expiry cron flips it, and matching it listed already-renewed members as "expiring" (July 2026 report). A stats panel shows TOTAL / ACTIVE / PENDING / **PAST EXPIRY (GRACE)** / EXPIRED / **EXPIRING (60d)** / NEW (30d) / RENEWALS counts (scoped to the chapter filter for area reps); all eight cards are anchor tags that drill down to a matching filter (Total clears filters, Active/Pending/Expired set `status`, Past expiry uses `expiring_within=overdue`, Expiring uses `expiring_within=60d`, New uses `created_range=30d`, Renewals uses `renewed=this_month`). Each `$statReset`s the other filters but preserves an active text search, and the card matching the current view gets a ring highlight. Power features:
...
`add.php` is a single page with seven client-side steps (vanilla JS, one final POST) — gated on `isFullAccess` **and** `canManualOrderFix`, and blocked for chapter-restricted accounts. It posts `action=create_member` via `fetch` with an `X-Requested-With: fetch` header; the handler answers JSON (`success` / `error` / member id + number + link) when that header is present, and falls back to the old redirect-with-flash flow otherwise. The header is deliberately the JSON trigger: a lapsed step-up replays the POST as a plain form (no header), so the replay takes the redirect path instead of rendering raw JSON, and the wizard's fetch detects the non-JSON step-up interception and re-submits natively. The handler:
...
2. Resolves the member number — Full/Life/unlinked-associate get their own base (suggested = `MAX(member_number_base)+1` honouring `membership.member_number_start`, overridable); a linked associate reuses the parent's base + `MAX(suffix)+1`. Duplicates are rejected. Before that, `MemberRepository::activeAssociateFor()` enforces the one-associate-per-member rule (see Ch 19); the same helper drives the greyed-out options in the picker, the `associate_taken` flag on `check_member_email`, and the `link_associate_member` action.
...
- **Email uniqueness is enforced on *change*, not on save.** `MemberRepository::isEmailAvailable($email, $excludeMemberId)` returns true when the member named by `$excludeMemberId` already holds that address, so a shared-household member (an associate on the full member's inbox) can still be edited. Without that, a name-only save on such a member was rejected with "That email address is already linked to another member" and could never be corrected — `save_profile` always posts the email field, even untouched. Creation paths pass no exclude id and stay strict; the only way to newly share an address is the wizard's `shared_email_ok` prompt.
