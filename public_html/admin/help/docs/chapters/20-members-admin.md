...
1. **Type & number** — Full / Associate / Life, chapter, and the membership number (the next free number is suggested; override if you need to). For an **Associate**, you can link them to an existing **Full** member — they then share that member's base number and get the next free suffix automatically (e.g. `1234.2`). Leave the link blank to give the associate their own number. Picking a link hides the member-number box — the number comes from the linked member, so there's nothing to type. **A member can only carry one associate at a time**: full/life members who already have one are greyed out in the picker and labelled with the associate's name, and the server rejects the submit as a backstop. Archiving (cancelling) an associate frees the slot; otherwise unlink the current one first (member page → Overview → Associate Member → **Unlink**).
