...
- **"AI request failed" appears.** Usually means the API key is wrong, the account is out of credits, or OpenRouter is down. Try once more; if it keeps failing, ask your developer to check the key in AI Settings.
...
The AI is powered by OpenRouter (openrouter.ai) — one account with prepaid credits that can run any major AI model. It costs real money per use — every chat message is billed against the association's account.

The key is configured in **Admin → Settings → AI Settings**. There's a **monthly cost cap** field — set it sensibly. It's a hard cut-off: when the month's spend reaches the cap, AI editing stops until the 1st of the next month.
...
- **The AI keeps failing or saying "request failed"** — your developer can check the API key, the OpenRouter credit balance, and status.openrouter.ai.
...
The provider is hard-locked to OpenRouter (openrouter.ai) — a gateway where one prepaid account and one API key reach every major model (DeepSeek, GPT, Gemini, Claude). One vendor relationship, per-call cost reporting, and the freedom to run a cheap model for routine copy edits. The lock is enforced in code: `AiProviderFactory::make()` returns `null` for any provider key other than `'openrouter'`, and the AI Settings UI labels the field "Provider is fixed to OpenRouter." It replaced kie.ai (a Claude-only reseller) in Jul 2026 — see [Appendix A — Decision log](view.php?slug=A-decision-log), decisions 6 and 15.

The model is free-choice: any model ID from [openrouter.ai/models](https://openrouter.ai/models), typed or picked from suggestions in Admin → Settings → AI (`config/app.php` → `ai.providers.openrouter.models` seeds the suggestion list). Default is `deepseek/deepseek-chat` — near-frontier quality for HTML work at a fraction of frontier pricing. Note DeepSeek has no vision: the design-reference-image feature needs a GPT/Gemini/Claude model.

### How it works

#### The services

- **`AiProviders/AiProviderFactory`** — the gate: returns an `OpenRouterProvider` only when the key `'openrouter'` is requested *and* a key is configured. Anything else returns `null`.
- **`AiProviders/OpenRouterProvider`** — POSTs to `https://openrouter.ai/api/v1/chat/completions` in the OpenAI chat format (string content or text/image_url part arrays passed straight through). Requests `usage: {include: true}` so OpenRouter reports the actual USD cost of every call in `usage.cost`, which feeds the monthly cap.
- **`AiProviderKeyService`** — reads/writes the encrypted API key in `ai_provider_keys.api_key_encrypted`, wrapped with `EncryptionService::encrypt` (AES-256-GCM, see [Chapter 10 — Encryption & secrets at rest](view.php?slug=10-encryption-secrets)). The factory tries this table first and falls back to `config('ai.providers.openrouter.api_key')` (the `OPENROUTER_API_KEY` env var).
...
One AI editing path: **HTML element/page edit** via `/admin/page-builder/api.php` (`ai_edit` and `create_page` actions). The endpoint builds the scope-locked system prompt inline (`ai_scope_lock_prompt()` + master prompt + guardrails), checks the monthly cap, calls the provider, parses the JSON reply, and drops the result into the draft via `PageBuilderService::replaceElementHtml()` (element mode) or `ensureDraftHtml()` (page mode). Each turn is logged to `page_chat_messages` and the call's cost to `ai_usage_monthly`.

Scope-locks apply to every call: no PHP, no SQL, no `<script>` / `<iframe>` / inline handlers, no references to settings/users/roles/files. The system prompt is the first line of defence; `ai_sanitize_html()` in the endpoint is the second.

> **History.** Until Jul 2026 a second, block-schema flow existed (`/api/pages.php` + `AiService` + `AiPageBuilderService` + `PageAiRevisionService` + `PageSchemaService` + `UnifiedDiffService`). It had no UI caller and bypassed the usage cap, so it was deleted. Its tables (`ai_conversations`, `ai_messages`, `page_ai_revisions`) still exist but nothing writes to them.
...
- **Versions drawer** — restore any published `page_versions` snapshot in one click; the restore becomes the new draft.
...
- **`config/app.php` → `ai`** — the model suggestion list, env fallback wiring. Edit-and-redeploy only.
- **`.env`** — `OPENROUTER_API_KEY`, `AI_DEFAULT_MODEL` as last-resort fallbacks if no DB key/setting is present.
...
- `ai.provider` — always `'openrouter'`, force-rewritten on every load/save of the AI settings page.
- `ai.model` — any OpenRouter model ID (default `deepseek/deepseek-chat`).
...
- The OpenRouter API key itself lives in `ai_provider_keys` (encrypted, `provider = 'openrouter'`), not in `settings_global`.
...
- **Every chat turn is a paid model call.** Even a one-word reply hits the API. `ai.monthly_cap_usd` is a *hard cut-off* — `usage_check_cap()` returns HTTP 429 once the month's recorded spend reaches it. But note it defaults to `0` (= no cap) until the AI Settings form has been saved at least once.
- **Key rotation can break the AI silently.** Replacing the OpenRouter key updates the encrypted row but doesn't test it. A bad key surfaces as "OpenRouter (401): …" in the chat panel. After rotating, send one test prompt.
- **Hard OpenRouter lock, free model choice.** `AiProviderFactory` returns `null` for anything other than `'openrouter'`. Switching *gateways* needs a new adapter + factory change; switching *models* is just typing a different ID in Settings → AI. A typo'd or retired model ID makes every AI call fail with an OpenRouter 400 — fix it in Settings.
- **Vision is model-dependent.** The reference-image feature sends OpenAI-style `image_url` parts; DeepSeek and other text-only models reject them with a provider error. Use GPT/Gemini/Claude when attaching design references.
...
- **Image generation is a stub.** `generate_image_ai()` in `api.php` always returns "not available"; the `ai.image_generation_enabled` toggle only unlocks a path that errors. Use the [media library](view.php?slug=25-media-library) instead.
...
<!-- SCREENSHOT: /admin/settings/ai.php showing provider locked to OpenRouter, model picker, monthly cap, guardrails. Save as 24-ai-settings.png. -->
<!-- ![AI Settings](../images/24-ai-settings.png) -->

## Related chapters

- [04 — Configuration & environment](view.php?slug=04-configuration) — `OPENROUTER_API_KEY`, `AI_DEFAULT_MODEL`, and the `config/app.php → ai` section.
- [10 — Encryption & secrets at rest](view.php?slug=10-encryption-secrets) — how the OpenRouter API key is wrapped with AES-256-GCM in `ai_provider_keys`.
