...
- **`StripeService`** (`app/Services/StripeService.php`) — low-level SDK wrapper. Every Stripe API call goes through here: `createCheckoutSession`, `createCheckoutSessionForPrice` (single Stripe price ID), `createCheckoutSessionWithLineItems` (ad-hoc `price_data` items — used by member renewal so amounts come from the pricing matrix, not pre-built Stripe price objects), `createCustomer`, `createRefund`, `retrievePaymentIntent`, `constructEvent`. Resolves the secret key internally, so callers never see one.
