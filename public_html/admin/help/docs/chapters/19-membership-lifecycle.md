...
- Clicking it opens the **renewal lightbox** — a focused, full-screen modal that handles the whole renewal in one place. Inside the lightbox they:
  1. Pick a **term** — the options come straight from the admin-defined renewal periods (ships with 1 Year and 3 Years; add/rename/remove them under Settings → Membership pricing). Each option shows the price for their member type and magazine preference; greyed-out options mean that term has no AUD amount set in the pricing matrix. Whatever length a period is set to is exactly how far the renewal date advances — a 3-year renewal always moves the date three whole years on, even if the member had already lapsed. One refinement (Aug 2026): a member who renews within the **renewal grace window** after the 31 July year end (default 3 months, so through 31 October — configurable in Settings → Membership pricing) is renewing the year that *just started*, so a 1-year renewal paid in early August ends at the **next** 31 July, not a year beyond it. Only renewals *after* the window run their full term from the coming year end.
...
3. **Check the bank.** Once the transfer has cleared, open the request and hit **Approve** or **Deny**:
   - **Approve** → activates the membership exactly like a card payment would: sets them active and sets the new renewal date from the term they chose (a 3-year renewal advances three whole years, etc.). Approvals in the first months after 31 July land on the *next* year end (renewal grace window — a 1-year renewal approved on 3 August 2026 ends 31 July 2027, not 2028), so the Treasurer no longer has to hand-correct early-August renewals. The member is emailed that their membership is active.
...
3. `MembershipService::createMembershipPeriod()` inserts a `membership_periods` row with `status = 'PENDING_PAYMENT'`. Expiry via `calculateExpiry()` is **always 31 July**, N years out (membership year is Aug–Jul). For **brand-new joins** the first-year end comes from `MembershipService::newJoinYearEnd()`: normally the current year's 31 July, but from the join-rollover date (default 1 June, see Ch 14) it's 31 July of the **next** year — a June/July joiner pays the start-of-year price and gets the remaining weeks free. `LIFE` rows have `end_date = NULL`.
...
6. Repeats 3–5 for the associate sub-application under the same `full_member_id` — unless that member already has a live associate, in which case the approval posts an error alert and skips the associate (one associate per member; see below).
...
Renewal **end dates** are derived at activation in `MembershipOrderService::activateMembershipForOrder()`: still-active cover stacks the term on top of the existing end date; lapsed/expired renewals run the term from `MembershipService::lapsedRenewalAnchor()` — the 31 July that just *passed* while inside the renewal grace window (`renewal_grace_months`, default 3 → through 31 Oct), or the *coming* 31 July after the window (never short). The one-shot `fix_short_renewal_dates.php` tool replays these same rules.

**Orders with no period (Aug 2026).** A membership order can exist with `membership_period_id` NULL — the admin **Request payment** top-up creates one deliberately, and a signup order carries none until approval. When such an order was paid by card, `activateMembershipForOrder()` returned `false` and `markMembershipPaid()` did nothing at all, silently: the order read *paid* in admin while the member had no membership period and so **no renewal date**. Activation now mints the missing period itself (`createPeriodForOrphanOrder()`), taking the term off the order's line items (`termFromOrderItems()` — a period key like `3Y` / `JOIN_P_3Y` wins, else the duration is read out of the prose, else `1Y`), then calculates the end date exactly as above. Three cases still return 0 rather than minting: a `PENDING` applicant (approval owns their period), a life member, and an order whose items don't mention membership/renewal/join (the Request-payment box is free text — a one-off charge must not extend cover). Members already stuck undated are repaired by the one-shot `fix_missing_renewal_dates.php`, which replays their latest **paid** membership order through the same path; the dry-run runs for real inside a rolled-back transaction, so the preview is the exact result.
...
#### Associate life members (Aug 2026)

A linked associate can also hold life membership (member 950.1 was the first). `member_type` **stays `ASSOCIATE`** — the household link (`full_member_id`) and `.1` member number depend on it — and the life status lives in `members.is_life_member = 1` (Migration 049). Every "never lapses / no renewal" rule must treat `member_type = 'LIFE'` **or** `is_life_member = 1` as life: use `MemberRepository::isLifeMember($member)` in PHP and `MemberRepository::notLifeSql($pdo)` in SQL (`m` alias, pre-migration safe). Both crons, the lapsed lockdown (`MembershipAccessService`), the member-portal renew/upgrade prompts, and the partner add-on in the renewal drawer already do. Set it from the member view's **Membership type** dropdown ("Associate Life") or a manual membership order with the "Associate Life" type — both write the flag and a `LIFE`-term period.
...
- **Changing a member's status in the single-member view locks them immediately.** `App\Services\MembershipAccessService::effectiveStatusFrom()` makes `members.status` authoritative for the locked states (`LAPSED`/`INACTIVE` — i.e. admin "Expired"/"Cancelled"/"Suspended", which `MemberRepository::normalizeStatus` maps to those legacy values) and only falls back to the live `membership_periods.status` for active/pending members. So an admin setting Expired/Cancelled/Suspended locks the member even while their period row still reads ACTIVE, but a freshly *renewed* member (already `members.status` ACTIVE) is never false-locked. Set them back to Active to restore access.
- **Lapsed lockdown is a UI gate, not a hard server gate.** A lapsed member (effective status `LAPSED`/`INACTIVE`/expired/cancelled/suspended, non-LIFE, no payment in flight) gets a red banner, a once-per-login lightbox, and — on community pages — a heavy blur overlay, all driven by `App\Services\MembershipAccessService` and the `app/Views/partials/member_lockdown.php` partial. The dashboard itself stays open, but its preview widgets for locked features (Wings, Upcoming Rides, AGM Awards, Notice Board, and the Members-directory quick action) each get their own small lock overlay via the `gw_dashboard_lock_overlay()` helper in `member/index.php` (CSS `.gw-lock-card-*` in the lockdown partial), so they can't be clicked through. Locked vs open is decided by `MembershipAccessService::LOCKED_PAGES` / `OPEN_PAGES` (open = dashboard, billing, profile, history, settings, help; everything else community/store locks). The partial is included right after the mobile-topbar include in each member entry point, keyed by `$lockdownPageKey`; the sidebar greys locked items but keeps them clickable on purpose. **The overlay is cosmetic — the page HTML still renders underneath.** For pages exposing other members' personal data (the directory especially) a lapsed member can still read it via view-source. If you need a *hard* block, render the overlay instead of the data, or use `AdminMemberAccess::requireActiveMember()` on that page. `require_login()` alone still lets a lapsed member in.
...
- **One associate per member.** `MemberRepository::activeAssociateFor($fullMemberId, $excludeId = null)` returns the associate holding a member's household slot, ignoring rows whose status is inactive / cancelled / archived (status is matched case-insensitively — the live column carries the legacy UPPERCASE values). Every path that attaches an associate checks it first: the add-member wizard (`create_member`), `link_associate_member`, `check_member_email` (drives the wizard's shared-email prompt), the application/renewal approval in `admin/index.php`, and the member-facing `/api/create-subscription`. Archiving an associate frees the slot; otherwise unlink them from the member's Overview tab. **The rule is enforced in application code only — there is no DB constraint**, so bulk imports and direct SQL can still create a second one, and pre-existing duplicates stay put until someone unlinks them.
