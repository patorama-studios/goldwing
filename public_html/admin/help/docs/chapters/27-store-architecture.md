...
| `/store/checkout` | `checkout.php` (routes to `/checkout`) | Shipping/pickup, on-page Stripe Payment Element for card entry, "Pay now" confirms a PaymentIntent (no off-site redirect for cards). Renders in the member-area shell (sidebar + topbar). Two-column layout: accordion sections (Contact → Shipping → Payment) on the left, sticky order summary on the right. |
| `/store/orders`, `/store/order/<order_number>` | `orders.php`, `order_view.php` | "My orders" + order detail / ticket codes. |
| `/order/success?order=<order_number>` | `public_html/order/success/index.php` | Post-payment thank-you screen — green check, order number, mini line-item summary, CTAs (Back to dashboard / Main website / View order details). Accepts `?orderId=<id>` for the legacy bank-transfer flow. |

`/store/*` and `/checkout` unconditionally call `require_login()` — the store is members-only and the `payments.stripe.allow_guest_checkout` setting is no longer consulted for store routes (it remains a no-op for backwards compatibility). Non-members hit the login page and return via `?return=/store/…`.
...
- **Lazy Stripe Product sync** — Stripe Products are created on demand the first time a store_product is referenced from a checkout. To prime the catalog before the first sale (or push name/description updates), use the admin tool `/admin/sync-store-products-to-stripe.php`. Dry run by default; `?confirm=1` to execute; `?confirm=1&force=1` to also update Products that are already synced. Gated by `admin.settings.general.manage`.

- **Sending a stuck order an invoice link** — `/admin/send-store-invoice-link.php?order=<order_number>` looks up a `store_orders` row, runs `StoreInvoiceService::ensureInvoiceForOrder()` to produce (or reuse) the matching Stripe Invoice, and surfaces the hosted invoice URL. Useful when (a) a customer's card declined on the on-page Element and you want to give them a Stripe-hosted retry link, (b) the order was taken offline, or (c) you've just released a stuck cart with the cleanup tool and want to email the customer their payment link instead of asking them to retry checkout. Includes a copy-URL button and a `mailto:` link pre-populated with the customer's email + a short message. Idempotent — pressing Generate twice reuses the existing draft/open invoice.
...
Webhooks land on `/api/stripe_webhook.php` and are dispatched by `PaymentWebhookService` ([Ch 16](view.php?slug=16-webhooks-idempotency)). The handler reads `metadata.order_type` and routes store events (`invoice.paid` is the primary event for the Invoice flow; `payment_intent.succeeded`, `checkout.session.completed`, and `charge.refunded` are still handled for legacy/fallback paths) to the store branch — marking the order paid, **converting the cart (`store_carts.status = 'converted'`)**, decrementing stock, generating ticket codes for ticket items, and sending the confirmation email. The cart is intentionally NOT marked converted at create-PaymentIntent time, so an abandoned attempt or declined card leaves the cart `active` and the user can retry from the same cart.
...
- **Store is members-only — always.** Every `/store/*` page and `/checkout` calls `require_login()` unconditionally; the old `payments.stripe.allow_guest_checkout` toggle no longer affects the store (it remains in the settings UI but is a no-op for store routes). Anonymous visitors land on `/login.php?return=/store/…`. The store also renders inside the member-area shell — sidebar with cart-count badge, topbar, no public-site nav.
