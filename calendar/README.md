...
- Event management uses role checks (`ADMIN`, `AREA_REP`). Role names are matched case-insensitively against `users.role`/`$_SESSION['user']['roles']` (see `calendar/lib/auth.php::calendar_require_role`).
