    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare('INSERT INTO calendar_events (title, slug, description, media_id, scope, chapter_id, event_type, timezone, start_at, end_at, all_day, recurrence_rule, rsvp_enabled, is_paid, ticket_product_id, capacity, sales_close_at, map_url, map_zoom, online_url, meeting_point, destination, status, created_by, created_at)
                VALUES (:title, :slug, :description, :media_id, :scope, :chapter_id, :event_type, :timezone, :start_at, :end_at, :all_day, :recurrence_rule, :rsvp_enabled, :is_paid, :ticket_product_id, :capacity, :sales_close_at, :map_url, :map_zoom, :online_url, :meeting_point, :destination, "pending", :created_by, NOW())');
            $stmt->execute([
                'title' => $title,
                'slug' => $slug,
            ...
            $success = 'Event submitted successfully! It will appear on the calendar once an admin approves it.';
        } catch (Throwable $e) {
            error_log('[Calendar] Event submission failed: ' . $e->getMessage());
            if ($eventId === 0) {
                $errors[] = 'Unable to submit event. Please try again later.';
            } else {
                $success = 'Event submitted successfully! It will appear on the calendar once an admin approves it.';
            }
        }
...
$pageTitle = 'Submit Calendar Event';
...
<div class="flex h-screen overflow-hidden">
  ...
  <main class="flex-1 overflow-y-auto bg-background-light relative">
    <?php $topbarTitle = 'Submit Event'; require __DIR__ . '/../../app/Views/partials/backend_mobile_topbar.php'; ?>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p class="text-sm text-gray-500">Dashboard / Calendar / Submit Event</p>
          <h1 class="text-2xl font-display font-bold text-gray-900">Submit an Event</h1>
        </div>
      ...
      <?php if ($success) : ?>
        ...
        <div class="mt-4"><a href="dashboard_events.php" class="text-primary font-semibold">Return to Dashboard</a></div>
      <?php else: ?>
        ...
        <div class="space-y-6">
          <section data-tour="submit-event-title" class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 space-y-4">
            ...
            <div>
              ...
              <textarea name="description" data-wysiwyg rows="6" placeholder="Describe the event details, itinerary, and requirements..." required><?php echo calendar_e($old('description')); ?></textarea>
            </div>
